  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "24px 16px 48px" }}>
        ...
        <h1 style={{ fontSize: 26, fontWeight: 800, color: "#1a1a1a", marginBottom: 24 }}>Checkout ({cartCount} items)</h1>
        ...
        {cart.length === 0 ? (
          ...
          <div className="izu-cart-grid" style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 32, alignItems: "start" }}>
            <div>
              <h3 style={{ fontSize: 16, fontWeight: 700, color: "#1a1a1a", marginBottom: 16 }}>Order Items</h3>
              {cart.map((item) => (
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <div style={{ display: "flex", alignItems: "center", border: "1px solid #e8e8e8", borderRadius: 6, overflow: "hidden" }}>
                        <button onClick={() => { if (item.quantity <= 1) removeFromCart(item.product.id); else updateQuantity(item.product.id, item.quantity - 1); }} style={{ border: "none", background: "#f5f5f5", padding: "4px 10px", cursor: "pointer" }}>−</button>
                        <span style={{ width: 36, textAlign: "center", fontSize: 13, fontWeight: 700 }}>{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.product.id, item.quantity + 1)} style={{ border: "none", background: "#f5f5f5", padding: "4px 10px", cursor: "pointer" }}>+</button>
                      </div>
                      <button onClick={() => removeFromCart(item.product.id)} style={{ border: "none", background: "none", color: "#dc2626", cursor: "pointer", padding: 4 }}>
            ...
            <div className="izu-cart-summary" style={{ background: "#fff", border: "1px solid #f0f0f0", borderRadius: 16, padding: 24, position: "sticky", top: 80 }}>
              <h3 style={{ fontSize: 18, fontWeight: 800, color: "#1a1a1a", marginBottom: 20 }}>Shipping Details</h3>
              ...
              {orderError && <div style={{ color: "#dc2626", fontSize: 12, fontWeight: 600, marginBottom: 8 }}>{orderError}</div>}

              <button onClick={handlePlaceOrder} disabled={placing}
                style={{ width: "100%", background: placing ? "#ccc" : "linear-gradient(135deg, #e8320a, #f05a28)", color: "#fff", border: "none", padding: "14px", borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: placing ? "not-allowed" : "pointer", boxShadow: "0 4px 12px rgba(232,50,10,0.25)" }}>
